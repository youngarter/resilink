...
export function DeleteResidenceDialog({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const { error: dbError } = await supabase.from("residences").delete().eq("id", residenceId)
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
          <p className="text-sm">
            Are you sure you want to delete <strong>{residenceName}</strong>?
          </p>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button variant="destructive" onClick={handleDelete} disabled={isLoading}>
              {isLoading ? "Deleting..." : "Delete Residence"}
            </Button>
          </div>
        </div>
      </DialogContent>
